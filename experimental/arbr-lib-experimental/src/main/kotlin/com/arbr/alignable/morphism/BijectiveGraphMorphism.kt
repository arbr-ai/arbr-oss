...
import com.arbr.platform.alignable.alignable.graph.AlignableHomogenousRootedDAG
import com.arbr.platform.alignable.alignable.graph.HomogenousRootedDAG
import org.slf4j.LoggerFactory
import reactor.core.publisher.Mono

abstract class BijectiveGraphMorphism<SourceType: Any, V, ParsedNodeType, E : Alignable<E, O>, O> {
    ...
    abstract fun buildGraphFromSourceObject(source: SourceType): HomogenousRootedDAG<V>

    abstract fun buildSourceObjectFromGraph(resultGraph: HomogenousRootedDAG<ParsedNodeType>): SourceType

    fun apply(source: SourceType): Mono<SourceType> {
        val pairedDAG: AlignableHomogenousRootedDAG<AlignableProxy<V, E, O>, O> = buildGraphFromSourceObject(source).flatMapAlignable {
        ...
        return targetGraphMono.flatMap { targetGraph ->
            val pairedTargetDAG: AlignableHomogenousRootedDAG<AlignableProxy<V, E, O>, O> = targetGraph.flatMapAlignable {
    ...
    companion object {
        private val logger = LoggerFactory.getLogger(BijectiveGraphMorphism::class.java)
    }
}
