...
class NoiseAdderV1 private constructor(
    ...
    private val contextLineFabricateProbability = 0.15
    private val contextLineDropProbability = 0.15
    ...
    private fun addTextNoise(patch: String, tokenInterjectionProbability: Double): String {
        val patchTokens = bytePairEncoder.encode(patch)
    ...
    private fun generateLine(after: String): String {
        val priorTokens = bytePairEncoder.encode(after)
    ...
    companion object {
        const val VERSION = "nv1"

        private const val vocabSizeCompressionRatio = 50
        private const val windowSize = 8

        private val random = Random(1328132909)

        private val logger = LoggerFactory.getLogger(NoiseAdder::class.java)

        fun train(
            documents: List<String>,
            parameterValueProvider: ParameterValueProvider,
        ): NoiseAdder {
            logger.info("Training noise adder on ${documents.size} documents totaling ${documents.sumOf { it.length } / 1000}KB...")

            val trainStartMs = Instant.now().toEpochMilli()

            val bytePairEncoder = BytePairEncoder.train(document = "", vocabSizeDivisor = vocabSizeCompressionRatio)
                .extendTraining(documents)

            logger.info("Trained Byte Pair Encoder")

            val documentTokenSequences = documents.map { bytePairEncoder.encode(it) }
